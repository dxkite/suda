...
class SQLStatementPrepare
{
    ...
    public function count(string $table, $where = '1', array $binds = [], string $what = '*'):int
    {
        $table = self::table($table);
        $where = self::prepareWhere($where, $binds);
        $sql = 'SELECT count('.$what.') as `count` FROM `'.$table.'` WHERE '.$where.';';
    ...
    public function nextId(string $table, string $database = null)
    {
        ...
        $table = self::table($table);
        if ($query = (new RawQuery($this->connection, $sql, ['database' => null === $database? conf('database.name'):$database,'table' => $table]))->fetch()) {
