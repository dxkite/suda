...
class Pinyin
{
    const  UTF8=1;
    const  GB2312=2;
    private static $pinyin_code=array(
    ...
    public static function getAll(string $chinese, string $delimiter = '', int $length = 0, int $charset=self::UTF8)
    {
        ...
        $py= preg_replace_callback('/([\x{4e00}-\x{9aff}]+)/u', function ($matchs) use ($delimiter, $charset) {
            return $delimiter.Pinyin::_pys($matchs[1], $delimiter, $charset).$delimiter;
        }, $chinese);
        $py=trim($py, $delimiter);
        if ($length) {
    ...
    private static function zh2py($num, $blank = '')
    {
        if ($num>0 && $num<160) {
            return chr($num);
            ...
            foreach (self::$pinyin_code as $py => $code) {
                if ($code > $num) {
                    break;
                }
                $result = $py;
    ...
    private static function _pys(string $chinese, string $delimiter = '', int $charset=self::UTF8)
    {
        if ($charset != self::GB2312) {
            $chinese = self::_U2_Utf8_Gb($chinese);
        }
        return self::zh2pys($chinese, $delimiter);
    }
}
